...
- **Масштаб времени** (petTimeScale) настраивается в Settings → «Масштаб вр.» (×1, ×10, ×60, ×100)
- Формула: `ageTickMs = 60000 / petTimeScale` (1 пет-минута = 60 пет-сек проходит за 60/scale реальных секунд)
- petTimeScale = 1: 1 сек реал = 1 сек питомца (AGE_TICK_MS = 60 000)
- petTimeScale = 60: 1 сек реал = 60 сек питомца (1 мин)
...
millis() ──► now
    ...
    └──► Внутри petTick:
         ...
         ageTimer      ── каждые (60000/scale) мс ──► ageMinutes +1
```
...
| ageTickMs | 60000 / petTimeScale (настраивается) |
...
### 14.3. Сравнение для TamaFi (настраивается в Settings)

| Масштаб | ageTickMs | До ELDER | Стиль |
|---------|-----------|----------|-------|
| ×1 | 60 000 | ~3 часа | Tamagotchi-подобный |
| ×10 | 6 000 | ~18 мин | Компромисс |
| ×60 | 1 000 | ~3 мин | Быстрый демо |
| ×100 | 600 | ~2 мин | Очень быстро |

### 14.4. Рекомендации

- **Продакшен / долгая игра:** ×1 (реальное время).
- **Демо / тесты:** ×60 (1 сек = 1 мин питомца).
- **Баланс:** ×10 (1 мин = 10 мин питомца).
